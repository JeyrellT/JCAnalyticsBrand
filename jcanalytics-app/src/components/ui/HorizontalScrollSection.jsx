            ...
            >
              <TiltCard className="w-full h-full max-w-md">
                <div className="bg-white rounded-[2rem] border border-slate-200/60 shadow-xl shadow-slate-200/50 transition-all duration-300 group flex flex-col h-full overflow-hidden relative">
                  ...
                  <div className={`absolute top-44 md:top-52 left-6 z-20 glass-card p-3 rounded-2xl border ${card.iconBox}`}>
                    {card.icon}
                  </div>
                  
                  <div className="p-6 md:p-8 pt-10 md:pt-12 flex-1 flex flex-col bg-white z-20 relative">
                    <h3 className={`font-display text-2xl font-bold mb-4 text-slate-900 transition-colors ${card.colorClass}`}>{card.title}</h3>
                    <p className="font-sans text-slate-600 mb-6 text-sm flex-1">
                      {card.desc}
                    </p>
                    <ul className="space-y-3 mb-6">
                      {card.points.map((point, i) => (
                        <li key={i} className="flex items-center gap-3 text-slate-700 font-bold bg-slate-50 p-3 rounded-xl border border-slate-100">
                          <CheckCircle size={20} className={`${card.checkColor} shrink-0`} /> 
                    ...
                    <button 
                      ...
                      className={`flex w-full items-center justify-center gap-2 text-center py-3 px-4 rounded-xl font-bold transition-all border ${card.btnClass}`}
                    >
                      Ver todos los casos reales <ArrowRight size={18} />
                    </button>
                  </div>
                </div>
              </TiltCard>
