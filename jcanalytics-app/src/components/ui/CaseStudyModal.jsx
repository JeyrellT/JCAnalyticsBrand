...
const CaseStudyModal = ({ isOpen, onClose, categoryId, casesData }) => {
  ...
  // animación de salida de AnimatePresence, lo que dejaba el modal atascado en
  // opacity:0 sin desmontar y con el scroll del body bloqueado.
  useEffect(() => {
  ...
  const category = isOpen && categoryId ? casesData[categoryId] : null;
  const Icon = category?.icon;

  // El portal se renderiza siempre; AnimatePresence envuelve el render condicional
  // para que la animación de salida (exit) se ejecute al cerrar.
